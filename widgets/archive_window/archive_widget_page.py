...
class Archive_widget_page(QGridLayout):
    ...
    def drow_archive_all(self):
        ...
        all_money = self.helper.get_list("""SELECT sum(cash), sum(card)
                                            FROM CloseOrderView;""")[0]
        if all_money[0] != None and all_money[1] != None:
            ...
            self.total_money.setText(f"total: {all_money[0]+all_money[1]}")
            for row in range(len(info)):
                self.archive_list.setCellWidget(row, 0, InfoProductsOrder(row = row, id_table = str(info[row][1]), db="Alpha.db"))# id_table
                ...
                self.archive_list.setCellWidget(row, 8, DelCloseOrderButton(text = "del", id_closeOrder=info[row][0], main_widget = self.main_widget)) # del



class Archive_widget_page2(QGridLayout):
    def __init__(self, main_widget):
        super().__init__()
        self.helper = Db_helper("Beta.db")
        self.archive_list = OrdersListWidget(active_window = ...)
        self.main_widget = main_widget
        self.archive_list.setColumnCount(9) 
        self.archive_list.add_columns(((0, "id_table"), (1, "name_table"), (2, "client_name"), (3, "cash"), (4, "card"), (5, "total"), (6, "time open"), (7, "time close"), (8, "") ))
    ...
    def drow_archive_all(self):
        ...
        all_money = self.helper.get_list("""SELECT sum(cash), sum(card)
                                            FROM CloseOrderView;""")[0]
        if all_money[0] != None and all_money[1] != None:
            info = self.helper.get_list(f"""SELECT * FROM HistoryTable WHERE {self.sorting.category_search} LIKE '%{self.quick_search.quick_search_line}%' GROUP BY id_table ORDER BY {self.sorting.category_search}""")
            self.archive_list.setRowCount(len(info))
            self.total_cash.setText(f"cash: {all_money[0]}")
            self.total_card.setText(f"card: {all_money[1]}")
            self.total_money.setText(f"total: {all_money[0]+all_money[1]}")
            for row in range(len(info)):
                self.archive_list.setCellWidget(row, 0, InfoProductsOrder(row = row, id_table = str(info[row][1]), db="Beta.db"))# id_table
                self.archive_list.setItem(row, 1, CustomQTableWidgetItem(str(info[row][2]))) # name_table
                self.archive_list.setItem(row, 2, CustomQTableWidgetItem(str(info[row][3]))) # client_name
                self.archive_list.setItem(row, 3, CustomQTableWidgetItem(str(info[row][6]))) # cash
                self.archive_list.setItem(row, 4, CustomQTableWidgetItem(str(info[row][7]))) # card
